...
(function(){
	shell = {
		...
		cmd : {
			...
			cpf : { // EXPERIMENTAL
				info : 'returns if is a valid CPF <http://en.wikipedia.org/wiki/Cadastro_de_Pessoas_F%C3%ADsicas>',
				run : function(n){
					var num = String(n).replace(/\.|\-/g,'');
					if (num.length === 11){
						var a = 0, b = 0, c = 11, d = 10;
						for (var i = 0; i <= 8; i++){
							a += d*num.charAt(i);
							b += c*num.charAt(i);
							d--;
						...
						if (a == num.charAt(9) && b == num.charAt(10)){
							printf('true');
						}
						else {
							printf('false');
						...
						printf('error: CPF value length must have 11 numbers only');
					}
				}
			}
		}
